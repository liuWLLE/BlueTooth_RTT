...
static void (*SendBuf)();       //发送函数
bool isUartRxCompleted;         //接收标志位
 ...
 * Function:    LobotDelay
 * Description: 简易ms延时
 * Parameters:  a:延时时间（ms）
 * Return:      无返回
 **********************************************************************************/
static void LobotDelay(uint32_t a)
{
    while(a--)
    {
...
{
    if(servoID > 31 || !(Time > 0))	return;         //舵机ID不能打于31,可根据对应控制板修改
...
void moveServosByArray(LobotServo servos[], uint8_t Num, uint16_t Time)
{
    uint8_t index = 7;
    uint8_t i = 0;
    if(Num < 1 || Num > 32 || !(Time > 0)) return;     //舵机数不能为零和大与32，时间不能为零
    LobotTxBuf[0] = LobotTxBuf[1] = FRAME_HEADER;      //填充帧头
    ...
    SendBuf(LobotTxBuf, LobotTxBuf[2] + 2);             //发送
}
...
void moveServos(uint8_t Num, uint16_t Time, ...)
{
    ...
    if (Num < 1 || Num > 32) return;                //舵机数不能为零和大与32，时间不能小于0
    LobotTxBuf[0] = LobotTxBuf[1] = FRAME_HEADER;   //填充帧头
    LobotTxBuf[2] = Num * 3 + 5;                    //数据长度 = 要控制舵机数 * 3 + 5
...
void runActionGroup(uint8_t numOfAction, uint16_t Times)
{
    ...
    SendBuf(LobotTxBuf, 7);                         //发送
    receiveHandle();                                //等待动作自然结束
}
 ...
 * Description: 获取开发板返回指令
 * Parameters:  无
 * Return:      无返回
 * Others:      循环中需要延时以待寄存器复位
 * 调试方案:
        LCD_ClearRect(30,80,70,70); 
        LCD_SetAsciiFont(&ASCII_Font32); 
        LCD_DisplayNumber(50,100,LobotRxBuf[3],1);
 **********************************************************************************/
void receiveHandle(void)
{
    while(true)
    {
        ...
        if(isUartRxCompleted)
        {
            ...
            if(LobotRxBuf[3] == CMD_ACTION_GROUP_COMPLETE)
            {
                LobotRxBuf[3] = 0;
                break;
            }
