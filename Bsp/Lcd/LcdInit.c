...
>>>>> 重要说明：
*
*   1.屏幕配置为16位RGB565格式，SPI驱动
*   2.1.69寸液晶模块（型号：SPI169M1-240*280），屏幕控制器 ST7789
*   3.SPI通信速度为 18M (如果觉得刷屏速度过慢，可以修改 LCD_SPI_Config() SPI的时钟 )
*
>>>>> 其他说明：
*
...
char DMA_Buff[LCD_DMA_BUF_SIZE];    //LCD缓冲区，16位宽（每个像素点占2字节）
...
******************************************************************************************/
static void LCD_Delay(uint32_t a)
{
    while(a--)
    {
...
*   返 回 值: 无
*   函数功能: 初始化LCD的GPIO口
*   说    明: 1、SPI1的源引脚是PA4、PA5、PA6、PA7，而PA15、PB3、PB4原本是SPI3的引脚，但SPI1是APB2上的设备，
                最高通信速率为36Mbits，而SPI2、SPI3是APB1上的设备，最高通信速率为18Mbits，所以不使用SPI3，
                而是将PB3、PB4重映射为SPI1，能使得刷屏速度更快
              2、用户可以把分频系数设置为2，将SPI时钟提高到36，可以大大提高刷屏速度，但是因为ST的103手册里，
                限定了SPI的最高速度为18M，所以需要根据您的实际工况去选择是否使用
******************************************************************************************/
static void LCD_Ctrl_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(GPIO_LCD_CS_CLK|GPIO_LCD_DC_CLK|
                           GPIO_LCD_BACKLIGHT_CLK|GPIO_LCD_SCK_CLK|
                           GPIO_LCD_SDA_CLK,ENABLE);                        //开启LCD控制引脚的时钟
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO,ENABLE);                     //开启IO口复用时钟
    RCC_APB2PeriphClockCmd (LCD_SPI_CLOCK,ENABLE);                          //开启SPI时钟
    RCC_AHBPeriphClockCmd(LCD_SPI_DMA_CLOCK,ENABLE);                        //开启DMA时钟

    GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable,ENABLE);                 //禁用JTAG只使用SWD,不然PA15、PB3、PB4无法正常使用
    ...
    GPIO_SetBits(LCD_CS_PORT,LCD_CS_PIN);                                   //拉高片选，禁止通信

    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_Out_PP;                       //推挽输出
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;                       //速度等级
    GPIO_InitStructure.GPIO_Pin = LCD_DC_PIN;                               //初始化DC引脚
    GPIO_Init(LCD_DC_PORT, &GPIO_InitStructure);            
    GPIO_SetBits(LCD_DC_PORT,LCD_DC_PIN);                                   //DC引脚拉高，默认处于写数据状态
            
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_Out_PP;                       //推挽输出
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;                       //速度等级
    GPIO_InitStructure.GPIO_Pin = LCD_BACKLIGHT_PIN;                        //初始化背光引脚
    GPIO_Init(LCD_BACKLIGHT_PORT, &GPIO_InitStructure);         
    GPIO_ResetBits(LCD_BACKLIGHT_PORT,LCD_BACKLIGHT_PIN);                   //先关闭背光，初始化完成之后再打开

    SPI_I2S_DeInit(LCD_SPI);                                                //复位SPI
    ...
    SPI_InitStructure.SPI_BaudRatePrescaler = LCD_SPI_CLOCK_RATE;           //SPI时钟为分频系数
    ...
    DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)&LCD_SPI_REG_DR;   //SPI数据寄存器地址
    DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)DMA_Buff;              //内存地址
    DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralDST;                      //方向(从内存到外设)
    DMA_InitStructure.DMA_BufferSize = LCD_DMA_BUF_SIZE;                    //传输内容的大小
    ...
    DMA_Init(LCD_SPI_DMA_CHANNEL,&DMA_InitStructure);                       //配置DMA通道
    
    DMA_Cmd(LCD_SPI_DMA_CHANNEL,DISABLE);                                   //失能DMA
}

/**************************************************************************
函数功能：开启一次DMA传输
入口参数：DMA_CHx：DMA通道、buff：数据项多少
返回  值：void
注意事项：1、DMA1与DMA2通用  2、接收与发送通用  3、DMA通道一旦使能就进行传输
//DMA_GetCurrDataCounter(DMA_CHx);  //得到当前还剩余多少个数据
**************************************************************************/
static void DMA_Transfer(DMA_Channel_TypeDef* DMA_CHx,u16 buff)
{
...
static void LCD_WriteCommand(uint8_t lcd_command)
{
    ...
    LCD_SPI_REG_DR = lcd_command;                   // 发送的数据
    DMA_Transfer(LCD_SPI_DMA_CHANNEL,1);            // DMA发送
...
{
    LCD_SPI_REG_DR = lcd_data>>8;                   // 发送数据，高8位
    DMA_Transfer(LCD_SPI_DMA_CHANNEL,1);            // DMA发送
    LCD_SPI_REG_DR = lcd_data;                      // 发送数据，低8位
    DMA_Transfer(LCD_SPI_DMA_CHANNEL,1);            // DMA发送
}
...
{
    LCD_SPI_REG_CR &= 0xFFBF;                       // 关闭SPI
    LCD_SPI_REG_CR |= SPI_DataSize_16b;             // 切换成16位数据格式
    LCD_SPI_REG_CR |= 0x0040;                       // 使能SPI
    LCD_CS_L;                                       // 片选拉低，使能IC
    for(uint32_t i = 0;i < DataSize;i++)
    {
        LCD_SPI_REG_DR = DataBuff[i];
        DMA_Transfer(LCD_SPI_DMA_CHANNEL,1);        // DMA发送
    }
    LCD_CS_H;                                       // 片选拉高
    LCD_SPI_REG_CR &= 0xFFBF;                       // 关闭SPI
    LCD_SPI_REG_CR &= 0xF7FF;                       // 切换成8位数据格式
    LCD_SPI_REG_CR |= 0x0040;                       // 使能SPI
}
...
{
   LCD_DrawLine_H(x,y,width);           // 绘制水平线
   LCD_DrawLine_H(x,y+height-1,width);
   
   LCD_DrawLine_V(x,y,height);          // 绘制垂直线
...
*   函数功能: 在指定区域内填充指定颜色块 
*
*   说    明: 1、要绘制的区域不能超过屏幕的显示区域  2、适配LVGL  3、SPI屏幕需要开启LV_COLOR_16_SWAP宏
*
******************************************************************************************/
void LCD_FillColor(uint16_t x1,uint16_t y1,uint16_t x2,uint16_t y2,uint16_t *color)
{
    ...
    LCD_SetAddress(x1,y1,x1+width-1,y1+height-1);       // 设置坐标
...
{  
    ...
    for(uint16_t i = 0; i <height; i++)                     // 循环按行写入
    {
